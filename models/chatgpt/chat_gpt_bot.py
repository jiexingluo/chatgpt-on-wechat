...
class CustomAIBot(ChatGPTBot):
    """
    A unified Bot inheriting ChatGPTBot specifically for Custom (OpenAI-Compatible) models.
    It completely segregates the user configuration fields (custom_model, custom_api_key, custom_api_base)
    to prevent them from polluting or overriding standard open_ai_api_key.
    Since it inherits from ChatGPTBot, parsing logic remains 100% compatible.
    """
    def __init__(self):
        super().__init__()
    ...
    def reply_text(self, session: ChatGPTSession, api_key=None, args=None, retry_count=0) -> dict:
        ...
        try:
            return super().reply_text(session, api_key=custom_key, args=args, retry_count=retry_count)
        finally:
