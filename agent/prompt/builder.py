...
def _build_workspace_section(workspace_dir: str, language: str, is_first_conversation: bool = False) -> List[str]:
    ...
    lines = [
        ...
        "**知识库文件位置**:",
        "",
        "所有知识库文件都存放在项目根目录的 `knowledge_base/memory/` 文件夹下。",
        "- 📁 查询知识库内容 → 使用 `rag_search` 工具（语义+关键词搜索，快速精确）",
        "- ❌ **禁止**使用 `bash find` 或 `bash grep` 搜索知识库 — 会超时且效率低下",
        "- 📄 读取特定KB文件 → 使用 `read` 工具直接读取 `knowledge_base/memory/文件名.md`",
        "",
        "**交流规范**:",
        "",
